...
/**
 * Get the Default Remote Candidate
 *
 * @param icem   ICE Media object
 * @param compid Component ID
 *
 * @return Default Remote Candidate address if set, otherwise NULL
 */
const struct sa *icem_rcand_default(struct icem *icem, uint8_t compid)
{
	const struct icem_comp *comp = icem_comp_find(icem, compid);

	if (!comp || !comp->def_rcand)
		return NULL;

	return &comp->def_rcand->addr;
}
