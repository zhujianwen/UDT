...
int CUDT::listen(UDTSOCKET u, const sockaddr* peer, int backlog)
{
	try
	{
		return s_UDTUnited.listen(u, peer, backlog);
	}
	catch (...)
	{
		return INVALID_SOCK;
...
int CUDT::postRecv(UDTSOCKET u, const sockaddr* addr, const char* buf, uint32_t len)
{
	try
	{
		return s_UDTUnited.postRecv(u, addr, buf, len);
	}
	catch (...)
	{
		return INVALID_SOCK;
